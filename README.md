...
| `chuleta sniper armar <marketId> <tope>` / `ver` / `limpiar` / `ejecutar` / `cron` | Francotirador fantasma: puja en los últimos segundos (valor + 10 si nadie puja; tu tope en el último minuto si hay rivales). **Los topes redondos se suben en mil**: los rivales pujan cifras redondas y un empate es una moneda al aire que se puede perder gratis. `cron` lee la hora del ciclo de tu liga y te da la línea de crontab. |
...
- `chuleta sniper armar|ver|limpiar|ejecutar|cron`: ghost-mode sniper that bids
  ...
  contested). Round caps are bumped by a thousand, since rivals bid round
  figures and a tie is a coin flip; `cron` reads your league's cycle time and
  prints the crontab line.
